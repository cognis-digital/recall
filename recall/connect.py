"""Native cognis-connect emit for recall — forward findings to any platform.

Maps recall's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    recall ... --format json | recall-emit --to stix
    recall-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['title'] = out.get('title', 'Untitled')
