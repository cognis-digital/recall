...
**recall** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `recall` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
recall ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
recall ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
recall ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
```
...
works whatever `recall` outputs:

```python
from cognis_connect import normalize, misp
findings = [normalize(rec, source="recall") for rec in records]   # records = your JSON output
