...
How **recall** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `recall` lives in the **Identity / persona** cluster.
...
| from | relation | to |
|---|---|---|
| `recall` | JSON findings compose with | [`personagraph`](https://github.com/cognis-digital/personagraph), [`corpmap`](https://github.com/cognis-digital/corpmap) |
| `recall` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `recall` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `recall` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the identity / persona cluster.** `recall` output feeds the next tool:
```bash
recall ... --format json > out.json          # this tool's findings (see `recall --help`)
...
recall ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
recall ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
recall ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `recall`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `recall` slots
